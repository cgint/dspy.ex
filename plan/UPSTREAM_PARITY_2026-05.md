...
| Duplicate input/output field names — `3278e0f8` | Signature construction rejects duplicate names within inputs, within outputs, and across input/output fields (for example `value -> value`). | Covered | P0 | `test/r3_1_core_contract_hardening_test.exs` | Implemented duplicate input/output field rejection and cross input/output distinctness in `Dspy.Signature.new/2`, which also covers DSL modules and string/arrow signatures. |
...
| BootstrapFewShot `seed=-1` + `metric_threshold` — `90a9dd81` | Passes `metric_threshold` in unshuffled seed=-1 path. | Covered | P1 | `test/r3_2_teleprompter_parity_test.exs`; existing: `test/bootstrap_few_shot_smoke_test.exs`, `test/teleprompt/bootstrap_few_shot_determinism_test.exs` | Added `metric_threshold` support and `seed: -1` natural-order bootstrapping; fixed bootstrapped chunk order preservation. |
| MIPROv2 demo index assignment — `0ae2ca09` | Fixes demo index assignment in optimizer trial logging (`raw_chosen_params` uses `demos_idx`, not `instruction_idx`). | Partial / not directly applicable | P1 | `test/r3_2_teleprompter_parity_test.exs`; existing: `test/teleprompt/mipro_v2_*` | Elixir MIPROv2 has a simplified config flow and no Python-style `trial_logs` / `raw_chosen_params`; deterministic test guards that compiled examples are stable and drawn from the training domain. |
| GEPA updates — `631085c0`, `73b26249`, `b3ee96fa`, `9e8a323d` | GEPA dependency/API moved: cached evals, removed tool optimization, result/docs changes. | Divergent / covered subset | P1 | `test/r3_2_teleprompter_parity_test.exs`; existing: `test/teleprompt/gepa_*`, `plan/GEPA.md` | Current `dspy.ex` GEPA remains an explicit toy finite-candidate instruction selector; deterministic tests cover supported subset. Full upstream GEPA parity remains out of scope without a separate architecture decision. |
