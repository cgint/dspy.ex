...
|---|---|---:|---:|---|---|
| ChainOfThought rationale semantics — `4ef729d2` | Restores pre-`dspy.Reasoning` legacy rationale-field behavior; configurable `rationale_field` / `rationale_field_type`; prepends `reasoning` field with string-like rationale by default. | Partial | P0 | Existing: `test/acceptance/chain_of_thought_acceptance_test.exs`, `test/acceptance/chain_of_thought_attachments_acceptance_test.exs` | Inspect `lib/dspy/chain_of_thought.ex`; add explicit parity test for rationale/reasoning field naming and default type. |
| Empty LM response adapter error — `b833bc55` | Adapters raise/return explicit parse error on empty/null LM response instead of producing silent `None`/success. | Unknown | P0 | Candidate: new adapter/Predict test | Inspect `Dspy.Signature.Adapter.Pipeline` and parser behavior; add deterministic mock LM test for empty response. |
| Duplicate input/output field names — `3278e0f8` | Signature construction rejects duplicate field names across input/output definitions. | Unknown | P0 | Candidate: `test/signature_test.exs` | Inspect DSL + string parser; add tests for module DSL and arrow/string signatures. |
| Input field type validation — `71493dba` | Python signatures validate input field types more strictly. | Partial | P0 | Existing: `test/signature_test.exs`, `test/signature_typed_schema_integration_test.exs` | Define Elixir-compatible scope: avoid surprising coercions; test required P0 types (`string`, `integer`, `float`, `boolean`) if feasible. |
...
| JSON diacritics preservation — `a3874e34` | JSON serialization uses `ensure_ascii=False`, preserving non-ASCII/diacritics. | Unknown | P0 | Candidate: adapter parsing/formatting test | Add deterministic test with non-ASCII output key/value content; verify Jason behavior and prompt formatting. |
