# Demonstrates Predict + MIPROv2 optimization + JSON-friendly parameter persistence.
#
# Run:
#   mix run examples/predict_mipro_v2_persistence_offline.exs

defmodule PredictMIPROv2PersistenceOfflineDemo do
  alias Dspy.{Evaluate, Example}
  alias Dspy.Teleprompt.MIPROv2

  defmodule MiproMockLM do
    @behaviour Dspy.LM
  ...
  def run do
    ...
    path =
      Path.join(
        System.tmp_dir!(),
        "dspy_predict_mipro_v2_params_#{System.unique_integer([:positive])}.json"
      )
    ...
    {:ok, restored} =
      Dspy.Module.apply_parameters(Dspy.Predict.new("question -> answer"), params2)
...
PredictMIPROv2PersistenceOfflineDemo.run()
